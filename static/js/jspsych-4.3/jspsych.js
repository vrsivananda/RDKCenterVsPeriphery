	...
	jsPsych = (function() {
		...
		function createExperimentChunk(chunk_definition, parent_chunk, relative_id){
			...
			chunk.next = function() {
				...
				if((this.type === 'if' || this.type == 'while') && typeof this.parentChunk !== 'undefined' && this.currentTimelineLocation >= this.timeline.length)
				{
					if(this.isComplete())
					{
						this.end();
						this.currentTimelineLocation = this.currentTimelineLocation - 1;
						this.parentChunk.advance();
						return this.parentChunk.next();
					}
					else
		...
		function doTrial(trial) {
			...
			jsPsych[trial.type].trial(display_element, trial);
		}
